...
- Math uses Upmath embedding (inline `$...$` / `\(...\)`, display `$$...$$` / `\[...\]` + `i.upmath.me/latex.js`), not KaTeX/MathJax; `normalizeLatexDelimiters` maps `\( \)` / `\[ \]` to dollars before parse; `remark-math` then protects setext/emphasis; `remarkMathToUpmath` emits literal `$$` for Upmath; script loads before React mount; preview re-processes `.markdown-body` (debounced); print waits for latex images.
